...
def main():
    ...
    # Startup menu for input method selection
    print("📋 Choose input method:")
    print("1. 🎤 Voice input (requires microphone)")
    print("2. ⌨️  Manual text input")
    print("3. 🔄 Auto-detect (try microphone, fallback to manual)")
    ...
    # Set microphone availability based on user choice
    if choice == '1':
        microphone_available = True
        force_microphone = True
    elif choice == '2':
        ...
        try:
            ...
            if microphone_available and consecutive_failures < 3:
                ...
                if recognized_text is None:
                    ...
                    if consecutive_failures >= 3 and not force_microphone:
                        ...
                        if choice in ['y', 'yes']:
                            ...
                            consecutive_failures = 0
                    elif force_microphone:
                        print("🔄 Microphone mode selected - trying again...")
                        consecutive_failures = 0
                else:
                    consecutive_failures = 0
            
            # Use manual input if microphone is not available or failed
            if not microphone_available or (recognized_text is None and consecutive_failures < 3 and not force_microphone):
            ...
            if recognized_text:
                ...
                # Ask if they want to switch input method (only if not forced)
                if microphone_available and not force_microphone:
                    switch_choice = input("🔄 Switch to manual text input? (y/n): ").lower().strip()
                    if switch_choice in ['y', 'yes']:
                        microphone_available = False
                elif not microphone_available and not force_microphone and consecutive_failures == 0:
